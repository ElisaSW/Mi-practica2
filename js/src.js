...
$.validator.addMethod("nif", function(value, element, param) {
  ...
  var dni = document.getElementById("dni").value.substring(0,8);
  var letters = "TRWAGMYFPDXBNJZSQVHLCKE";
  var resto = dni % 23;
  var letter = letters.substr(resto, 1);
  console.log(letter);
  if (resultado !== null && $("#dni").val().substring(8,9) === letter ){
      return true;
  }
 else {
   return false;
...
function change_env_class() {
  var env_class = document.getElementById("env_class").value;
  if (env_class != "") {
...
function load_coupons() {
  // JSON desde URL
  coupons = JSON.parse(coupons_data);
  print_coupons();
}
...
$("#boton").on("click", function() {
  $.ajax({
    ...
    var str = item.title.toUpperCase().includes($("#finder").val().toUpperCase());
...
function cargar_mapa(){
  ...
  map = new mapboxgl.Map({
    ...
    center: [-3.70325,40.4167], // starting position [lng, lat]
